...
import { endsWithNegativeSign, endsWithOperator, isOperator, } from './utils/validator'
...
function App() {
  ...
  function handlerDecimal() {
    if (evaluate) {
      ...
      if (!currentValue.includes('.')) {
        if(endsWithOperator.test(formula) || (currentValue == '0' && formula === '')
        ) {
          setCurrentValue('0.')
  ...
  function handleOperator({ target: { innerText }}) {
    if (evaluate) {
